...
- (void)execute:(CIRDatabase*)database
{
	...
	CIRResultSet* resultSet = [database executeQuery:@"SELECT version FROM migrations_schema"];
	
	while ([resultSet next]) [_migrations removeObjectForKey:@([resultSet longLongAtIndex:0])];
	
	if ([_migrations count] > 0)
	{
		CIRStatement* statement = [database prepareStatement:@"INSERT INTO migrations_schema (version) VALUES (?)"];
